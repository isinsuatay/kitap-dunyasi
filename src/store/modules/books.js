...
const mutations = {
  ...
  deleteBook(state, bookId) {
    ...
    if (bookToDelete) {
      const hasOtherBooksInCategory = state.books.some(
        (book) => book.category === bookToDelete.category
      );
      if (!hasOtherBooksInCategory) {
        state.categories = state.categories.filter(
          (category) => category !== bookToDelete.category
        );
      }

      const hasOtherBooksInLanguage = state.books.some(
        (book) => book.language === bookToDelete.language
      );
      if (!hasOtherBooksInLanguage) {
        state.languages = state.languages.filter(
          (language) => language !== bookToDelete.language
        );
      }
    }
...
const actions = {
  ...
  addBook({ commit, rootState }, book) {
    ...
    console.log("Mevcut Kullanıcı ID:", rootState.user?.id);
...
const getters = {
  ...
  sortedBooks: (state) =>
    computed(() =>
      sortBooks(state.filteredBooks || state.books, state.sortOption)
    ),
  isFavorite: (state) => (bookId) => state.favoriteBooks.includes(bookId),
  getBooksByUser: (state) => (userId) =>
    state.books.filter((book) => String(book.addedBy) === String(userId)),
};
